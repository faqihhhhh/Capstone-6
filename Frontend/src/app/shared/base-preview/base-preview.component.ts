...
export class BasePreviewComponent extends BaseComponent implements OnInit {
  ...
  getIsDownloadFlag(document: DocumentView) {
    this.sub$.sink = this.commonService
      .isDownloadFlag(this.data.isVersion ? document.id : document.documentId, this.data.isFromPreview)
      .subscribe((c) => {
      ...
      (event) => {
        if (event.type === HttpEventType.Response) {
          this.addDocumentTrail(
            documentView.isVersion ? documentView.id : documentView.documentId,
