...
export class DocumentVersionHistoryComponent extends BaseComponent implements OnInit {
  ...
  restoreDocumentVersion(version: DocumentVersion) {
    this.sub$.sink = this.commandDialogService
      .deleteConformationDialog('Are you sure you want to restore this to current version')
      .subscribe((isTrue) => {
        if (isTrue) {
          this.sub$.sink = this.documentService.restoreDocumentVersion(this.data.id, version.id).subscribe(() => {
  ...
  downloadDocument(version: DocumentVersion) {
    ...
    this.sub$.sink = this.commonService.downloadDocument(doc.id, true).subscribe(
      (event) => {
        if (event.type === HttpEventType.Response) {
          this.addDocumentTrail(this.data.id, DocumentOperation.Download.toString())
