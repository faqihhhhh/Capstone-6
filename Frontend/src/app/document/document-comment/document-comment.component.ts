...
export class DocumentCommentComponent extends BaseComponent implements OnInit {
  ...
  getDocumentComment() {
    this.sub$.sink = this.documentCommentService.getDocumentComment(this.data.id).subscribe((c: DocumentComment[]) => {
  ...
  onDelete(id: string) {
    this.sub$.sink = this.commonDialogService
      ...
      .subscribe((isTrue: boolean) => {
        if (isTrue) {
          this.sub$.sink = this.documentCommentService.deleteDocumentComment(id).subscribe(() => {
            this.getDocumentComment()
          })
        }
      })
