...
export class NotificationListComponent extends BaseComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit() {
    ...
    this.sub$.sink = merge(this.sort.sortChange, this.paginator.page)
      .pipe(
        tap(() => {
          this.notificationResource.skip = this.paginator.pageIndex
          this.notificationResource.pageSize = this.paginator.pageSize
          this.notificationResource.orderBy = this.sort.active + ' ' + this.sort.direction
          this.dataSource.loadNotifications(this.notificationResource)
        })
      )
      .subscribe()
  ...
  viewDocument(notification: UserNotification) {
    this.sub$.sink = this.notificationService.markAsRead(notification.id).subscribe(() => {})
    this.sub$.sink = this.documentService.getDocument(notification.documentId).subscribe((data: DocumentInfo) => {
