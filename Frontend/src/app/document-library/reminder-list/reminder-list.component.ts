...
export class ReminderListComponent extends BaseComponent implements OnInit {
  ...
  displayedColumns: string[] = ['action', 'documentName', 'startDate', 'endDate', 'subject', 'message', 'frequency']
  ...
  ngOnInit(): void {
    ...
    this.sub$.sink = this.filterObservable$.pipe(debounceTime(1000), distinctUntilChanged()).subscribe((c) => {
